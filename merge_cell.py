...
def init_argpasre():
    ...
    parser.add_argument('-i', '-input', nargs='?', type=str, help='input file folder', default=r'data\sim_data')
    parser.add_argument('-o', '-output', nargs='?', type=str, help='output file folder', default=r'data\merge_data')
    return parser
...
if __name__ == '__main__':
    ...
    print("Start to merge_cells...")
    merge_cell_name_list_path=args.m
    merge_cell_name_list=read_cell_name_list(merge_cell_name_list_path)


    start=datetime.datetime.now()

    if (len(merge_cell_name_list)==0):
        exit(0)
    else:
        ...
        merge_chr_pos_file=os.path.join(args.o,'chr_pos')
        raw_acp_df.to_csv(merge_chr_pos_file, sep='\t')
